...
// Các import cần thiết cho FS, Path, và URL
import { promises as fs } from 'node:fs';
import { dirname, join } from 'node:path'; // Bổ sung 'join'
...
// Helper function to resolve package paths in ES modules
function resolvePackagePath(packagePath) {
    // Simple path resolution for node_modules
    return join(process.cwd(), 'node_modules', packagePath);
}
...
// Đã loại bỏ các interface TypeScript, giữ lại cấu trúc đối tượng JS
...
const sources = {
    ...
    json: [
        ...
        {
            filename: '@iconify-json/fa/icons.json',
            icons: [
                'circle',
            ],
        },
...
(async function () {
    ...
    if (sources.json) {
        for (let i = 0; i < sources.json.length; i++) {
            ...
            // Load icon set
            const packagePath = typeof item === 'string' ? item : item.filename;
            const filename = resolvePackagePath(packagePath);
            const content = JSON.parse(await fs.readFile(filename, 'utf8')); // Bỏ type assertion 'as IconifyJSON'

            // Thay đổi thuộc tính stroke-width cho tabler icons nếu cần
