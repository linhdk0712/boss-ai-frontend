...
export const canNavigate = (to: any) => {
  try {
    ...
    // If no specific permissions defined, allow access (default behavior)
    // This handles routes without explicit CASL permissions
    const hasAnyPermissionRequirement = to.matched.some((route: any) =>
      route.meta?.action && route.meta?.subject
    )

    if (!hasAnyPermissionRequirement) {
      return true // Allow access to routes without specific permission requirements
    }

    // If there are permission requirements, check them
    return to.matched.some((route: any) => {
