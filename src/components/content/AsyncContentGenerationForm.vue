    ...
    <v-card-text class="py-4">
      <v-form ref="contentForm" @submit.prevent="handleGenerate">
        ...
        <VTextField
          ...
          hint="Optional title for your content"
          persistent-hint
          variant="outlined"
          :disabled="disabled"
          class="mb-3"
        ...
        <VTextarea
          ...
          hint="Describe what content you want to generate"
          persistent-hint
          class="mb-3"
          :disabled="disabled"
          :readonly="false"
        ...
        <div
          class="d-flex justify-space-between text-caption text-medium-emphasis mb-4"
        >
        ...
        <v-alert
          v-if="!canGenerate && content.trim().length > 0"
          type="info"
          variant="tonal"
          class="mt-3"
        >
          <v-alert-title>Ready to Generate!</v-alert-title>
          Please select Content Type and Language in the configuration panel to
        ...
        >
          ...
          <div class="completions-list">
            <v-chip
              v-for="completion in recentCompletions.slice(0, 3)"
              :key="completion.id"
              :color="completion.status === 'completed' ? 'success' : 'error'"
              size="small"
              variant="tonal"
              class="me-2 mb-2"
              @click="$emit('view-result', completion)"
            >
              <v-icon
                start
                :icon="
                  completion.status === 'completed' ? 'mdi-check' : 'mdi-alert'
                "
              />
              {{
